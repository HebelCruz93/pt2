<?php
    ...
    $idClienteR = $_POST['razonSocial'];   
    $fechaR = $_POST['fechaEmision']; 
    $fechaPagoR = $_POST['fechaPago']; 
    $totalR = $_POST['total'];
...
/*
    ...
    $queryRegistro ="INSERT INTO ingreso (noFactura, idCliente, fecha, fechaPago, total) 
    VALUES (:noFacturaP, :idClienteP, :fechaP, :fechaPagoP, :totalP) ";

    $resultado=$dbConexion->prepare($queryRegistro);
    $resultado->execute(array(":noFacturaP"=>$noFacturaR,                               
                                ":idClienteP"=>$idClienteR,                               
                                ":fechaP"=>$fechaR,
                                ":fechaPagoP"=>$fechaPagoR,                               
                                ":totalP"=>$totalR                                                               
                                ));
    echo "<script>alert('¡Ingreso registrado con exito!');</script>";
    echo "<script> window.location.href = '../vista/admin/visualizarIngreso.php'; </script>";
                            //}
