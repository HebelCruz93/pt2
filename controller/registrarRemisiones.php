...
/*
    ...
    $queryConsultaFactura = "SELECT noFactura FROM ingreso WHERE noFactura = :noFacturaC";
  
    $consulta = $dbConexion->prepare($queryConsultaFactura);
     
// Ejecucion sentencia preparada
$consulta->execute(array(":noFacturaC"=>$noFacturaR));


if ($registro=$consulta->fetch(PDO::FETCH_ASSOC)) {

  
    $queryConsultaRemision = "SELECT noFactura FROM remision WHERE noFactura = :noFacturaC";
  
    $consulta = $dbConexion->prepare($queryConsultaRemision);
     
// Ejecucion sentencia preparada
    $consulta->execute(array(":noFacturaC"=>$noFacturaR));
    if ($registro=$consulta->fetch(PDO::FETCH_ASSOC)) {
        echo "<script>alert('¡El numero de factura ya se encuentra en las remisiones!');</script>";
        echo "<script> window.location.href = '../vista/admin/visualizarRemision.php'; </script>";
    }else{
        $queryRegistro ="INSERT INTO remision (noFactura, motivo) 
        VALUES (:noFacturaR, :motivoR) ";
    
        $resultado=$dbConexion->prepare($queryRegistro);
        $resultado->execute(array(":noFacturaR"=>$noFacturaR,                               
                                    ":motivoR"=>$fechaR                              
                                                                                             
                                    ));
        echo "<script>alert('¡Remision registrada con exito!');</script>";
        echo "<script> window.location.href = '../vista/admin/visualizarRemision.php'; </script>";
    }
}else{
 echo "<script>alert('¡El numero de factura no se encuentra registrada en la base de datos!');</script>";
 echo "<script> window.location.href = '../vista/admin/visualizarRemision.php'; </script>";
}
