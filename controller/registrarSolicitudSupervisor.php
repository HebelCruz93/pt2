<?php
    require 'conexion.php';
     session_start();
    $IdAsuntoR = $_POST['IdAsunto'];
    $fechaR = $_POST['fecha'];  
    $solicitudR = $_POST['solicitud'];   
...
/*
    ...
    $queryRegistro ="INSERT INTO solicitud (idAsunto, fecha, solicitud, idEmpleado, idEstatus) 
    VALUES (:idAsuntoP, :fechaP, :solicitudP, :idEmpleadoP,:idEstatusP) ";

    $resultado=$dbConexion->prepare($queryRegistro);
    $resultado->execute(array(":idAsuntoP"=>$IdAsuntoR,                               
                                ":fechaP"=>$fechaR,                              
                                ":solicitudP"=>$solicitudR,
                                ":idEmpleadoP"=>$idEmpleadoR,
                                ":idEstatusP"=> $IdStatusR                                                              
                                ));
    echo "<script>alert('¡Solicitud generada con exito!');</script>";
    echo "<script> window.location.href = '../vista/supervisor/supervisorVisualizarSolicitud.php'; </script>";
                            //}
