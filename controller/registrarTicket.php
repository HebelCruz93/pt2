<?php
    require 'conexion.php';
     session_start();
    $idProductoR = $_POST['idProducto'];
    $cantidadR = $_POST['cantidad'];  
    $idMaterialR = $_POST['idMaterial'];   
    $idEmpleadoR = $_POST['idEmpleado']; 
    $nombreCompletoUsuarioR = $_POST['nombreCompletoUsuario']; 
    $IdStatusR = 1;
...
/*
    ...
    $queryRegistro ="INSERT INTO ticket (idProducto, cantidad, idMaterial, idEmpleado, generadoPor, idEstatus) 
    VALUES (:idProductoP, :cantidadP, :idMaterialP, :idEmpleadoP, :generadoPor,:idEstatusP) ";

    $resultado=$dbConexion->prepare($queryRegistro);
    $resultado->execute(array(":idProductoP"=>$idProductoR,                               
                                ":cantidadP"=>$cantidadR,                               
                                ":idMaterialP"=>$idMaterialR,
                                ":idEmpleadoP"=>$idEmpleadoR,                               
                                ":generadoPor"=>$nombreCompletoUsuarioR,
                                ":idEstatusP"=> $IdStatusR                                                              
                                ));
    echo "<script>alert('¡Ticket generado con exito!');</script>";
    echo "<script> window.location.href = '../vista/admin/generarTicket.php'; </script>";
                            //}
